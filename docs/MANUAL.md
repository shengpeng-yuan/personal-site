...
- **语言切换**「中文 / English」：切换语言并停留在当前页面；
- **主题按钮**（右上角，三态循环，点一下换一个）：时钟 = 跟随时间、太阳 = 亮色、月亮 = 暗色。
  默认是「跟随时间」—— 按访客本地时间与后台设置的时段自动决定；一旦手动选择，就一直按你的选择显示。
  鼠标悬停在按钮上会显示当前处于哪个状态。

### 6.2 进入后台管理

**方式一（常规）**：直接访问 `https://me.heartgo.top/admin`

**方式二（前台隐藏入口）**：页面上看不到任何入口按钮，访客不会发现

| 触发方式 | 操作 | 适用 |
| --- | --- | --- |
| 键盘密语 | 在**任意前台页面**依次敲入 `admin`（注意：焦点不能在输入框里） | 电脑 |
| 连点头像 | 在**首页**连续点击右侧那张头像名片 **5 次**（3 秒内） | 手机 / 电脑 |

两种方式都会跳转到后台；未登录时会先跳到登录页。

> ⚠️ 这只是**便利入口**，不是安全措施。任何前端隐藏手段都能被懂技术的人找到，
> 真正的防护是登录密码 + 后台会话校验。所以不需要也不应该把它当成机密。
>
> 想换密语或点击次数：改 [components/SecretAdminEntry.tsx](../components/SecretAdminEntry.tsx) 顶部的
> `PASSPHRASE` / `CLICK_COUNT` / `CLICK_WINDOW_MS` 常量，然后重新 `npm run build` 并重启。
...
项目          项目列表与编辑器
留言          访客留言（有未读时会显示红色角标）
站点设置      站点信息、备案信息、主题时段、个人介绍、技能、联系方式、修改密码
```
...
| **备案信息** | ICP 备案号 + 链接 | 页脚右下角。链接指向工信部备案系统（默认 `https://beian.miit.gov.cn/`）。**链接留空则只显示文字、不可点击** |
| **备案信息** | 公安备案号 + 链接 | 页脚右下角，带盾牌图标。链接指向全国互联网安全管理服务平台，通常是 `https://beian.mps.gov.cn/#/query/webSearch?code=你的备案号数字` |
| **主题** | 启用按时段自动切换 | 关闭后改为跟随访客的系统深色模式偏好 |
| **主题** | 亮色时段开始 / 结束 | 访客首次进入时按**访客本地时间**判断：落在该时段内显示亮色，其余时间显示暗色。支持跨午夜（如 20:00 → 06:00） |
